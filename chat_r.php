...
<head>
  ...
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
    ...
    <body style="margin: 10px; background-color: #fcc329;">
        ...
        ?>
        <h2 style = 'font-family: sans-serif; color: black; text-align: center;'><?php
        ...
        <form action="chat_r.php" method="post">
            <input autofocus="autofocus" type="text" name="message" style='width: 100%; display: inline-block; border: 2px solid black; background-color: #f7e8c0; outline: none; color: #0eccb5'>
            <br>
            <br>
            <input type="submit" name="send" value="Send" style = ' display: inline-block; background-color: #9a2afc; color: #1269e2; border: 2px solid blue; border-radius: 10px; height: 40px; width: 100%; font-size: 27px;'>
        </form>
        <br>
        <?php
        ...
        if ($r = mysql_query($get_messages)) {
            while ($row = mysql_fetch_array($r)) {
                if($row['username'] == $_SESSION['username']){
                    ...
                    print "<div style='background-color: #f9257a; border-radius: 10px; display: inline-block; text-align: right;'><p style='#2d2d2d; font-family: arial; font-size: 25px; margin: 5px; color: #28eafc;'>" . $row['message'] . "</p></div>";

                    $date = date_create($row['date_entered']);

                                    print "<p style='color: #9a2afc;'>" . date_format($date, "g:i A, M d") . "</p>";
                    ...
                    print "<div style='text-align: center; background-color: #5bb4e1; border-radius: 10px; display: inline-block;'><p style='background-color: #f79825; color: #28eafc; font-family: arial; font-size: 25px; margin: 5px;'>" . $row['message'] . "</p></div>";

                    $date = date_create($row['date_entered']);

                                    print "<p style='color: #9a2afc; text-align: center;'>" . date_format($date, "g:i A, M d") . "</p>";


                    print "</div><br>";
                } else {
                    print "<div style='float: left; width: 190px;'><br>";
                    print "<div style='background-color: #f79825; border-radius: 10px; display: inline-block; text-align: left;'><p style='color: #6ce15b; font-family: arial; font-size: 25px; margin: 5px;'>" . $row['message'] . "</p></div>";
...
border-bottom-left-radius: 10px; background-color: #9a2afc;">
    <div>
        ...
        <a href='chat.php'><span style="color: #a01064;" class="glyphicon glyphicon-chevron-left" aria-hidden="true" id='backButton'></span></a>
                <a href="chat_setting.php"><span class="glyphicon glyphicon-cog" aria-hidden="true" style="float: right; font-size: 35px; color: #a01064; margin-top: 10px; margin-right: 10px;"></span></a>
