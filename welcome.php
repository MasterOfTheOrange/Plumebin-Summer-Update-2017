...
border-bottom-left-radius: 10px; background-color: #9a2afc;">
    <div class="container">
        ...
        <a href='chat.php'><span style="color: #a01064;" style="color: #a01064;" class="glyphicon glyphicon-comment" aria-hidden="true" id="alpaca"></span></a>
          ...
          while ($row = mysql_fetch_array($r)) {
              if (!empty($row['feed']) and !empty($row['feed_name'])) {
                $text_date = date_create($row['date_entered']);
                  ...
                  print "<div style='background-color: #f9257a; color: #28eafc;' class='well' id='wellbox'><h3>" . $row['feed'] . "<strong><h3 style='color: #19b6ff;'><a href=\"other_user.php?username={$row['feed_name']}\">" . $row['feed_name'] . "</a></h3></strong></p>";
                  print "</h3>";
